...
export default function CategoriesOfCategoryList() {
    ...
    const loadTheCategoriesOfCategory = async () => {

        try {

            const response = await fetch(`http://localhost:4000/cat_cat/${params.id}`)
    ...
    const handleDelete = async (cat_id_1, cat_id_2) => {
        ...
        SetCategorias(categorias.filter(categoria => categoria.cat_id_2 !== cat_id_2))

        try {
            //borra la id seleccionada en el back end
            const resp = await fetch(`http://localhost:4000/remove/cat_cat/${cat_id_1}/${cat_id_2}`, {
                method: 'DELETE'
            })
    ...
    //recibe parametro para BORRAR la id seleccionada en el backend
    const handleAdd = async (cat_id_1, cat_id_2, name, type) => {

        try {
            //AGREGA la id seleccionada en el back end
            const resp = await fetch(`http://localhost:4000/add/cat_cat/${cat_id_1}/${cat_id_2}`, {
                method: 'POST',
            })
            //console.log(pat_id);
    ...
    return (
        <>
            <h2>Listado de Categorías hijas de la Categoría: {params.id}</h2>
            {
                        ...
                        >
                            <CardContent style={{
                                ...
                                <div style={{ color: 'white' }}>
                                    ...
                                    <Typography>{'Cat Id: ' + categoria.cat_id_2}</Typography>
                                ...
                                <div>
                                    ...
                                    <Button
                                        ...
                                        onClick={() => navigate(`/categories_category/${categoria.cat_id_2}`,
                                             params.id = categoria.cat_id_2
                                            , loadTheCategoriesOfCategory()
                                    ...
                                    <Button
                                        ...
                                        onClick={() => handleDelete(categoria.cat_id_1, categoria.cat_id_2)}
                                        style={{
                                            marginLeft: '.5rem'
                                        }}
                                    >
                ...
                (busquedadecategorias.length !== 0) ?
                    ...
                    busquedadecategorias.map(categoria => (
                        <Card
                            ...
                            key={categoria.name + categoria.cat_id_2}
                        >
                            <CardContent style={{
                                display: 'flex',
                                justifyContent: 'space-between'
